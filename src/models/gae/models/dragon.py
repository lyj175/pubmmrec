...
#         train_interactions = dataset.inter_matrix(form='coo').astype(np.float32)#TODO 交互边，元组方式存储（1,2）表示交互的item与user
...
#             # TODO 对比学习预编码
#             self.clVencoder = ClMLP.ClEncoder(self.v_feat, self.device,self.dim_latent)
#             # self.v_feat,gae_loss_v = self.clEncoder(self.v_feat,self.train_interactions)
#
#             self.v_feat,gae_loss_v = self.clVencoder.item_modal_forward(self.v_feat,adj,self.num_user,self.device)
#             # self.v_feat = self.clVencoder.item_modal_forward(self.v_feat,self.dataset.get_adj_matrix(),self.num_user,self.device)
...
#             # self.t_feat,gae_loss_t = self.clEncoder(self.t_feat,self.dataset.get_adj_matrix())
#             self.t_feat, gae_loss_t = self.clTencoder.item_modal_forward(self.t_feat, adj,self.num_user,self.device)
#
...
#             # 假如self.mm_adj = [[1.0, 0.5, 0.2, 0.1, 0.0],显然类似一个残差链接，当前物品本身的特征加上与其他物品相似度权重求和特征
#               # [0.5, 1.0, 0.3, 0.2, 0.1],
#               # [0.2, 0.3, 1.0, 0.4, 0.2],
#               # [0.1, 0.2, 0.4, 1.0, 0.3],
#               # [0.0, 0.1, 0.2, 0.3, 1.0]]
...
#         return pos_scores, neg_scores
#
#     # TODO 对比学习损失：计算正例分数与负例分数和之比的负对数
...
#
#         return loss_value + reg_loss +cl_loss
#
...
#         x = torch.cat((self.preference, features), dim=0).to(self.device)#TODO 前user_num行都是表示用户的向量（传播前随机初始化），后面都是item的向量
