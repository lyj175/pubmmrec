...
class TrainDataLoader(AbstractDataLoader):
    ...
    def get_adj_matrix(self):
        #TODO 返回i-u交互的稀疏邻接矩阵。现有交互数据格式为（0,1）...提取第一列也就是用户节点、边起点。
        # 提取第一列也就是item节点、边终点。**最终构建的是user数乘item数的方阵，所以item索引要加上user数，否则会冲突
        if self.adj != None:
            return self.adj
        rows = self.dataset.df[self.dataset.uid_field].apply(lambda x: copy.deepcopy(x))
        cols = self.dataset.df[self.dataset.iid_field].apply(lambda x: copy.deepcopy(x))
        cols += self.dataset.user_num
        self.adj = coo_matrix((np.ones(len(rows)), (rows, cols)),
