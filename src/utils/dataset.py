...
class RecDataset(object):
    def __init__(self, config, df=None):
        ...
        self.splitting_label = self.config['inter_splitting_label']#TODO x_label
    ...
    def load_inter_graph(self, file_name):
        inter_file = os.path.join(self.dataset_path, file_name)#TODO 加载交互数据集
        cols = [self.uid_field, self.iid_field, self.splitting_label]#TODO 数据表头，用户ID，itemid，数据操作标签
        self.df = pd.read_csv(inter_file, usecols=cols, sep=self.config['field_separator'])#TODO 根据上一句的表头读取填充数据
    ...
    def split(self):
        ...
        for i in range(3):#TODO 按照x_label划分数据集？
